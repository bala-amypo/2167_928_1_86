...
public class SuggestionServiceImpl implements SuggestionService {
    ...
    public Suggestion generateSuggestion(Long farmId) {
        ...
        List<Fertilizer> fertilizers = catalogService.findFertilizersForCrops(cropNameList);
        String fertilizerNames = fertilizers.stream().map(Fertilizer::getName).collect(Collectors.joining(","));

        Suggestion suggestion = Suggestion.builder()
                .farm(farm)
                .suggestedCrops(cropNames) // Empty string if list is empty [cite: 231, 268]
